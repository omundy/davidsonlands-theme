...
var animalsQuestionObj = {
    ...
    "hard": {
        "food":"Which of the following describes this animal's eating habit?",
...
function selectQuestions(count) {
    ...
    for (let i = 0; i < randomAnimals.length; i++) {
        ...
        // randomize answers
        q.possibleAnswers = shuffle(q.possibleAnswers);
...
function updateFunFact(index=null){
    if (index == null){
        ...
        $('.alert-info').html("<b>Fun fact:</b> "+ current.questions[index]);
        $('.alert-info').show();  
    }
}
